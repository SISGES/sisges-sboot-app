...
Este documento descreve a arquitetura relacional do banco de dados do SISGES, considerando **PostgreSQL**, **consultas SQL tipadas com pgx** e o **runner de migrations em Go**, com foco nas funcionalidades iniciais:
...
O DDL completo está nas migrations SQL em `db/migration/`:
...
## 5. Tipos de domínio e consultas Go

Os tipos de request/response e as consultas SQL estão em `internal/api`. O backend não mantém grafos de entidades em memória: cada operação seleciona somente as colunas necessárias e monta respostas tipadas.
...
Relacionamentos N:N são consultados diretamente pelas tabelas `teacher_class`, `class_discipline` e `discipline_teacher`. Isso elimina proxies, carregamento implícito e retenção de entidades associadas.

---

## 6. Convenções do backend Go

- **Schema:** todas as consultas qualificam explicitamente o schema `sisges`.
- **Nomenclatura:** tabelas e colunas usam `snake_case`; JSON mantém o contrato `camelCase` do frontend.
- **Migrations:** o runner aplica SQL versionado e reconhece o histórico legado do Flyway.
- **Identidade:** PostgreSQL `SERIAL`/`GENERATED BY DEFAULT AS IDENTITY`; IDs são obtidos com `RETURNING`.
- **Memória:** consultas de lista possuem limites explícitos, requests têm tamanho máximo e o pool inicia sem conexões ociosas.
...
- Documentação oficial: PostgreSQL, Go `database/sql`, pgx e práticas de migrations versionadas.
