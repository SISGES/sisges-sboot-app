...
public class SchoolClassSpecification {
    ...
    public static Specification<SchoolClass> withFiltersForTeacher(SchoolClassSearchRequest request, Integer teacherId) {
        return (Root<SchoolClass> root, CriteriaQuery<?> query, CriteriaBuilder cb) -> {
            List<Predicate> predicates = new ArrayList<>();
            Join<SchoolClass, Teacher> teachers = root.join("teachers");
            predicates.add(cb.isNull(root.get("deletedAt")));
            predicates.add(cb.equal(teachers.get("id"), teacherId));
            if (request != null) {
                if (request.getName() != null && !request.getName().isBlank()) {
                    predicates.add(cb.like(cb.lower(root.get("name")),
                            "%" + request.getName().toLowerCase() + "%"));
